...
class ECOTInference:
    def __init__(
        ...
        # ).cuda()
        self.processor = AutoProcessor.from_pretrained(saved_model_path, trust_remote_code=True)
        self.vla = AutoModelForVision2Seq.from_pretrained(saved_model_path, torch_dtype=torch.bfloat16,trust_remote_code=True,low_cpu_mem_usage=True).cuda()
    ...
    def step(
        ...
        action, generated_ids = self.vla.predict_action(**inputs, unnorm_key="bridge_orig", max_new_tokens=1024)
